...
st.set_page_config(page_title="Company Profiles", page_icon=None, layout="wide")
inject_base_styles()
...
tabs = st.tabs([name.upper() for name in PROFILES.keys()])

for tab, (company, profile) in zip(tabs, PROFILES.items()):
    with tab:
        st.markdown(f"### {company}")
        st.markdown(f"**Narrative:** {profile['narrative']}")
        ...
        with col2:
            ...
            st.markdown(
                f"""
                <div class="fact-card" style="border-left-color:#D4A72C;">
                {profile['our_reading']}
                </div>
                """,
                unsafe_allow_html=True,
