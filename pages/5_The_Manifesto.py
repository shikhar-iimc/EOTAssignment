...
st.set_page_config(page_title="The Manifesto", page_icon="\U0001F4DC", layout="wide")
inject_base_styles()
...
for (num, headline, body, theme) in filtered:
    contested = "\u26a0\ufe0f " if "Contested" in theme else ""
    st.markdown(
        f"""
        <div class="fact-card">
        <b>Point {num}.</b> {headline}<br>
        <span style="color:#5B5B5B;">{body}</span><br>
        <span class="tag tag-interp" style="margin-top:0.4rem;">{contested}{theme}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
